...
class RelatorioVendasEcommerceController extends Controller
{
    public function index(Request $request)
    {
        if (!$request->filled('data_inicio')) {
            return view('vendas_Ecommerce', ['cards' => [], 'pagination' => null]);
        }

        $inicio = $request->get('data_inicio');
        $fim = $request->get('data_fim');
        $empresa = $request->get('empresa');
        $lojaAlvo = 'E-COMMERCE'; // LOJA TRAVADA AQUI
        ...
        if ($empresaFiltro === 'Amissima') {
            $colunaFaturamento = "ecommerce"; // Coluna amissima no banco

        } elseif ($empresaFiltro === 'Syssa') {
            $colunaFaturamento = "syssa"; // Coluna syssa no banco

        } else {
            // Se for "Todas as Marcas", somamos as duas
            $colunaFaturamento = "ISNULL(ecommerce, 0) + ISNULL(syssa, 0)";
        }
        ...
        return view('vendas_Ecommerce', compact('cards', 'pagination', 'resumo'));
    }
}