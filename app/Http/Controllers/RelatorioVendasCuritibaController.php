...
class RelatorioVendasCuritibaController extends Controller
{
    public function index(Request $request)
    {
        if (!$request->filled('data_inicio')) {
            return view('vendas_Curirtiba', ['cards' => [], 'pagination' => null]);
        }

        $inicio = $request->get('data_inicio');
        $fim = $request->get('data_fim');
        $empresa = $request->get('empresa');
        $lojaAlvo = 'Curitiba'; // LOJA TRAVADA AQUI
        ...
        return view('vendas_Curirtiba', compact('cards', 'pagination', 'resumo'));
    }
}