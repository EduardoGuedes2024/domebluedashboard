...
class RelatorioVendasShowroomController extends Controller
{
    public function index(Request $request)
    {
        if (!$request->filled('data_inicio')) {
            return view('vendas_Showroom', ['cards' => [], 'pagination' => null]);
        }
        ...
        if ($empresaFiltro === 'Amissima') {
            $colunaFaturamento = "showroom"; // Coluna amissima no banco

        } elseif ($empresaFiltro === 'Syssa') {
            $colunaFaturamento = "showroom_syssa"; // Coluna Syssa no base64_encode
        ...
        return view('vendas_Showroom', compact('cards', 'pagination', 'resumo'));
    }
}