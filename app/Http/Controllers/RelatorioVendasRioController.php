...
class RelatorioVendasRioController extends Controller
{
    public function index(Request $request)
    {
        if (!$request->filled('data_inicio')) {
            return view('vendas_Rio', ['cards' => [], 'pagination' => null]);
        }

        $inicio = $request->get('data_inicio');
        $fim = $request->get('data_fim');
        $empresa = $request->get('empresa');
        $lojaAlvo = 'RJ'; // LOJA TRAVADA AQUI
        ...
        return view('vendas_Rio', compact('cards', 'pagination', 'resumo'));
    }
}