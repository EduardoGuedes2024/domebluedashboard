...
<body>
    <div class="header">
        ...
        <div class="periodo">Período Analisado: {{ date('d/m/Y', strtotime($data_inicio)) }} até {{ date('d/m/Y', strtotime($data_fim)) }}</div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            @php $percentual = $totalGeral > 0 ? ($venda->total_vendas / $totalGeral) * 100 : 0; @endphp
            <tr>
                <td><span class="uf-badge">{{ $venda->entrega_uf }}</span> Estado de {{ $venda->entrega_uf }}</td>
                <td class="text-right">R$ {{ number_format($venda->total_vendas, 2, ',', '.') }}</td>
                <td class="text-center">{{ number_format($percentual, 1, ',', '.') }}%</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
    ...
    <div class="footer mt-5">
        Relatório gerado em {{ date('d/m/Y H:i') }} | Sistema DOME BLUE Dashboard
    </div>
